...
router.post('/singin', jsonParser, function(req,res)  {
    ...
    mysqlConnection.sequelize.query("SELECT U.ENCRYPTED_USER_PASSWORD,     U.USER_ID,    U.USER_NAME,    U.DESCRIPTION,    U.EMAIL_ADDRESS,    U.LOGIN_CMS,    E.EMPLOYEE_NUM,    E.L1_ORGANIZACION,    E.L2_DEPARTAMENTO,    E.L3_AREA,    E.L4_GERENCIA,    E.L5_UNIDAD,    E.L6_PLATAFORMA,    E.L7_CARGO,  A.area,  P.plataforma as PLATAFORMA,  L.unidad AS UN,  E.IDREL_SUPERVISOR,    SUPERVISOR.DESCRIPTION AS NOM_SUPERVISOR,    SUPERVISOR.EMAIL_ADDRESS AS EMAIL_SUPERVISOR,    E.IDREL_COORDINADOR,    COORDINADOR.DESCRIPTION AS NOM_COORDINADOR,    COORDINADOR.EMAIL_ADDRESS AS EMAIL_COORDINADOR,    JEFE_SUP.USER_ID AS ID_JEFE_SUP,    JEFE_SUP.DESCRIPTION AS NOM_JEFE_SUP,    JEFE_SUP.EMAIL_ADDRESS AS EMAIL_JEFE_SUP    FROM APPLPORDB.USERS U    INNER JOIN EMPLOYEES E ON U.USER_ID=E.EMPLOYEE_ID    INNER JOIN APPLPORDB.L6_PLATAFORMA P ON P.id_auto=E.L6_PLATAFORMA  INNER JOIN APPLPORDB.L3_AREA A ON A.id_auto=E.L3_AREA INNER JOIN APPLPORDB.L5_UNIDAD L ON L.id_auto=E.L5_UNIDAD LEFT JOIN APPLPORDB.USERS SUPERVISOR ON SUPERVISOR.USER_ID=E.IDREL_SUPERVISOR    LEFT JOIN APPLPORDB.USERS COORDINADOR ON COORDINADOR.USER_ID=E.IDREL_COORDINADOR    LEFT JOIN APPLPORDB.EMPLOYEES EMPSUPERVISOR ON EMPSUPERVISOR.EMPLOYEE_ID=SUPERVISOR.USER_ID    LEFT JOIN APPLPORDB.USERS JEFE_SUP ON JEFE_SUP.USER_ID=EMPSUPERVISOR.IDREL_SUPERVISOR    WHERE (    (U.END_DATE = '0000-00-00 00:00:00')    AND ((U.BLOCKED_ACCESS > NOW()) or (U.BLOCKED_ACCESS = ?))    AND (U.USER_NAME='dcolmedo' AND  U.ENCRYPTED_USER_PASSWORD=MD5(?))    )",
    {
